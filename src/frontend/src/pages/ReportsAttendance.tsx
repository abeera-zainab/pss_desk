...
import type { AttendanceReportDTO } from "@shared/types";
...
function ReportsAttendanceKeyframes() {
  return (
    <style>{`
      @keyframes fadeUp {
...
export default function ReportsAttendance() {
  ...
  function load() {
    ...
    setError("");
    api
      .getAttendanceReport({ month, userId: selectedUserId || undefined })
      .then((data) => {
        setReport(data);
        if (!selectedUserId) {
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 text-sm flex items-center gap-2" style={{ color: "#64748B" }}>
                  ...
                  Hours and attendance for {month.replace('-', ' ')}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              onClick={() => api.downloadReport("attendance", "pdf", { month, userId: selectedUserId || undefined })}
            >
            ...
            <button
              className="flex items-center gap-1.5 rounded-2xl border bg-white px-4 py-2 text-sm font-medium transition-colors duration-200 hover:bg-gray-50"
              style={{ borderColor: "#E2E8F0", color: "#64748B" }}
              onClick={() => api.downloadReport("attendance", "xlsx", { month, userId: selectedUserId || undefined })}
            >
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-gray-50/50" style={{ borderColor: "#E2E8F0" }}>
                  ...
                  <th className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: "#64748B" }}>
                    <div className="flex items-center gap-2">
                      <FontAwesomeIcon icon={faUserCheck} className="text-[10px] text-blue-400" />
                      On Leave
                    </div>
                  </th>
                  <th className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: "#64748B" }}>
                    <div className="flex items-center gap-2">
                      <FontAwesomeIcon icon={faHourglassHalf} className="text-[10px] text-amber-400" />
                      Half Day
                    </div>
                  </th>
                  <th className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: "#64748B" }}>
              ...
              <tbody className="divide-y" style={{ borderColor: "#E2E8F0" }}>
                ...
                {filteredData?.map((row, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3" style={{ color: "#3B82F6" }}>
                      {row.onLeave}
                    </td>
                    ...
                    <td className="px-4 py-3 font-medium" style={{ color: "#1A1D23" }}>
                      {formatDurationMinutes(row.hoursMinutes)}
                    </td>
                  </tr>
                ))}
              </tbody>
